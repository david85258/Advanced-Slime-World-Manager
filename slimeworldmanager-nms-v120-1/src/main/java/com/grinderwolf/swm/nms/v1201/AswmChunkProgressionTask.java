...
public final class AswmChunkProgressionTask extends ChunkProgressionTask {
    ...
    private ReentrantAreaLock schedulingLock;
    ...
    protected AswmChunkProgressionTask(final ChunkTaskScheduler scheduler, final ServerLevel world, final int chunkX, final int chunkZ,
        ...
        try {
            Field field = ChunkTaskScheduler.class.getDeclaredField("schedulingLockArea");
            field.setAccessible(true);

            schedulingLock = (ReentrantAreaLock) field.get(this.scheduler);
        } catch (Throwable e) {
    ...
    public void schedule() {
        ...
        ReentrantAreaLock.Node lock = this.schedulingLock.lock(0, 0, 500);
        try {
            if (this.scheduled) {
            ...
            this.schedulingLock.unlock(lock);
        }
    ...
    public void cancel() {
        ...
        ReentrantAreaLock.Node lock = this.schedulingLock.lock(0, 0, 1);
        try {
            this.cancelled = true;
        } finally {
            this.schedulingLock.unlock(lock);
...
//                }
                ...
                v1201SlimeWorld slimeWorld = ((CustomWorldServer) this.world).getSlimeWorld();
                LevelChunk chunk;
